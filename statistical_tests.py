...
Performs Paired t-tests between Transformer and baseline models.
Calculates p-values, Cohen's d effect sizes, and exact t-distribution confidence intervals.
...
def paired_ttest(
    ...
    """
    Perform Paired t-test between Transformer and baseline (better for N=5).
    """
    ...
    try:
        statistic, p_value = stats.ttest_rel(transformer, baseline)
    ...
    # Calculate exact t-distribution 95% Confidence Interval
...
def print_results_table(results: Dict[str, Dict]):
    ...
    print(f"\n{'Comparison':<25} {'Mean Diff':<12} {'p-value':<12} {'Sig':<6} {cohen_label:<12} {'CI 95%':<22} {'Interpretation'}")
    print("-" * 105)
    ...
    for model_name, result in results.items():
        ...
        print(f"{comparison:<25} {mean_diff:+.4f}      {p_val:<12.4f} {sig:<6} {effect:+.4f}      {ci_str:<22} {result['effect_size_interpretation']}")

    print("-" * 105)
    print("\nSignificance: *** p<0.001, ** p<0.01, * p<0.05, n.s. = not significant")
    print("Effect Size:  |d|<0.2 Negligible, <0.5 Small, <0.8 Medium, ≥0.8 Large")
    print("CI 95%:       Exact t-distribution confidence interval for mean difference")
    print("=" * 105)
...
def generate_latex_table(results: Dict[str, Dict]):
    ...
    latex.append("\\caption{Paired t-test Results: Transformer vs. Baseline Models}")
    latex.append("\\label{tab:statistical_tests}")
    latex.append("\\begin{tabular}{lcccc}")
    latex.append("\\toprule")
    latex.append("Model & Mean Diff & p-value & Cohen's $d$ & 95\\% CI \\\\")
