...
def split_data_kfold(texts, k_folds=5, seed=42):
    ...
    fold_size = len(texts_shuffled) // k_folds
    folds = []
    
    for i in range(k_folds):
        start_idx = i * fold_size
        end_idx = start_idx + fold_size if i < k_folds - 1 else len(texts_shuffled)
        
        val_texts = texts_shuffled[start_idx:end_idx]
        train_texts = texts_shuffled[:start_idx] + texts_shuffled[end_idx:]
